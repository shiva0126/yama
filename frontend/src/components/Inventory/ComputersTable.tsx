  ...
  return (
    <div>
      <div className="flex flex-wrap items-center gap-3 border-b border-white/8 p-4">
        ...
        <span className="ml-auto whitespace-nowrap text-xs text-slate-500">{filtered.length} / {computers.length}</span>
      </div>
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-white/6">
            ...
            {!isLoading && filtered.length === 0 && <tr><td colSpan={6} className="py-8 text-center text-slate-500">No computers found.</td></tr>}
            {filtered.map((c, i) => (
              <tr key={i} className="transition-colors hover:bg-white/[0.03]">
                <td className="px-4 py-2.5">
                  ...
                  <p className="text-slate-500 text-xs">{c.dns_host_name}</p>
                </td>
                <td className="px-4 py-2.5 text-xs text-slate-300">{c.operating_system || '—'}</td>
                <td className="px-4 py-2.5">
                  <span className={clsx('text-xs px-2 py-0.5 rounded-full', c.enabled ? 'bg-emerald-500/20 text-emerald-300' : 'bg-slate-700 text-slate-400')}>
                    {c.enabled ? 'Enabled' : 'Disabled'}
                  </span>
                </td>
                <td className="px-4 py-2.5">
                  <div className="flex flex-wrap gap-1">
                    {c.is_domain_controller && <span className="text-xs bg-sky-500/20 text-sky-300 px-1.5 py-0.5 rounded">DC</span>}
                    {!c.laps_enabled && !c.is_domain_controller && <span className="text-xs bg-amber-500/20 text-amber-300 px-1.5 py-0.5 rounded">No LAPS</span>}
                    {c.trusted_for_delegation && <span className="text-xs bg-red-500/20 text-red-300 px-1.5 py-0.5 rounded">Unconstrained</span>}
                  </div>
                </td>
                ...
                <td className="px-4 py-2.5">
                  {(c.trusted_for_delegation || (!c.laps_enabled && !c.is_domain_controller)) && (
                    <span className={clsx('text-xs font-medium', c.trusted_for_delegation ? 'text-red-300' : 'text-amber-300')}>
