  ...
  return (
    <div>
      <div className="flex items-center gap-3 border-b border-white/8 p-4">
        ...
        <span className="ml-auto whitespace-nowrap text-xs text-slate-500">{filtered.length} / {groups.length}</span>
      </div>
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-white/6">
            {isLoading && <tr><td colSpan={5} className="py-8 text-center text-slate-500">Loading...</td></tr>}
            {!isLoading && filtered.length === 0 && <tr><td colSpan={5} className="py-8 text-center text-slate-500">No groups found.</td></tr>}
            {filtered.map((g, i) => (
              <tr key={i} className="transition-colors hover:bg-white/[0.03]">
                ...
                <td className="px-4 py-2.5 text-xs text-slate-400">{g.group_scope}</td>
                <td className="px-4 py-2.5 text-xs">
                  <span className={clsx('font-mono', g.members?.length > 50 ? 'text-amber-300' : 'text-slate-300')}>{g.members?.length ?? 0}</span>
                </td>
                <td className="px-4 py-2.5">
                  {g.is_privileged && <span className="text-xs bg-red-500/20 text-red-300 px-2 py-0.5 rounded-full">Privileged</span>}
                  {g.privilege_level && g.privilege_level !== 'none' && !g.is_privileged && (
                    <span className="text-xs bg-amber-500/20 text-amber-300 px-2 py-0.5 rounded-full">{g.privilege_level}</span>
                  )}
                </td>
                <td className="px-4 py-2.5 text-xs text-slate-500 max-w-xs truncate">{g.description}</td>
              </tr>
            ))}
          </tbody>
        </table>
