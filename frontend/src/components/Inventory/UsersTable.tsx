...
function SortHeader({ label, field, current, dir, onClick }: {
  ...
  return (
    <th
      className="px-4 py-3 font-medium cursor-pointer select-none hover:text-slate-200 transition-colors"
  ...
  return (
    <div>
      <div className="flex items-center gap-3 border-b border-white/8 p-4">
        ...
        <span className="ml-auto whitespace-nowrap text-xs text-slate-500">{filtered.length} / {users.length}</span>
      </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-white/6">
            ...
            {!isLoading && filtered.length === 0 && <tr><td colSpan={7} className="py-8 text-center text-slate-500">No users found.</td></tr>}
...
function UserRow({ user }: { user: ADUser }) {
  ...
  if (user.password_never_expires) flags.push({ label: 'No Expiry', cls: 'bg-sky-500/20 text-sky-300' })
  if (user.is_service_account) flags.push({ label: 'Service', cls: 'bg-slate-500/20 text-slate-300' })

  return (
    <tr className="transition-colors hover:bg-white/[0.03]">
      <td className="px-4 py-2.5 font-mono text-xs text-white">{user.sam_account_name}</td>
      <td className="px-4 py-2.5 text-xs text-slate-300">{user.display_name}</td>
      <td className="px-4 py-2.5 text-xs text-slate-400">{user.last_logon ? new Date(user.last_logon).toLocaleDateString() : '—'}</td>
      <td className="px-4 py-2.5 text-xs text-slate-400">{user.pwd_last_set ? new Date(user.pwd_last_set).toLocaleDateString() : '—'}</td>
          ...
          {[user.is_privileged, user.dont_require_preauth, user.service_principal_names?.length > 0, user.password_never_expires].filter(Boolean).length > 0 && (
            <div className="flex gap-0.5">
              {[user.is_privileged, user.dont_require_preauth, user.service_principal_names?.length > 0, user.password_never_expires].map((v, i) => (
                <div key={i} className={clsx('w-1.5 h-3 rounded-sm', v ? 'bg-red-400' : 'bg-slate-700')} />
              ))}
            </div>
          )}
        </div>
      ...
      <td className="px-4 py-2.5">
        <span className={clsx('text-xs px-2 py-0.5 rounded-full', user.enabled ? 'bg-emerald-500/20 text-emerald-300' : 'bg-slate-700 text-slate-400')}>
