-- Migration 003: Complete defense plane schema + agent defense columns
-- Safe to run against an existing DB: all statements use IF NOT EXISTS / ADD COLUMN IF NOT EXISTS.

-- ============================================================
-- Agents: add defense columns if not already present
-- ============================================================
ALTER TABLE agents ADD COLUMN IF NOT EXISTS defense_mode BOOLEAN NOT NULL DEFAULT FALSE;
ALTER TABLE agents ADD COLUMN IF NOT EXISTS defense_url  TEXT;

-- ============================================================
-- Defense plane tables (identical to 002 but wrapped in DO
-- so the script is idempotent when 002 already ran)
-- ============================================================
...
-- ============================================================
-- Indexes (IF NOT EXISTS guards make these idempotent)
-- ============================================================
...
CREATE INDEX IF NOT EXISTS idx_defense_response_actions_incident  ON defense_response_actions(incident_id);
CREATE INDEX IF NOT EXISTS idx_defense_evidence_bundles_incident  ON defense_evidence_bundles(incident_id);

-- Missing indexes from 001 on ad_domain_controllers and inventory_snapshots
CREATE INDEX IF NOT EXISTS idx_ad_dc_snapshot_id   ON ad_domain_controllers(snapshot_id);
CREATE INDEX IF NOT EXISTS idx_ad_dc_scan_id        ON ad_domain_controllers(scan_id);
CREATE INDEX IF NOT EXISTS idx_inv_snapshots_scan_id ON inventory_snapshots(scan_id);
